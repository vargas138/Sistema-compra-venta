...
function agregarCarrito(parametro){
    ...
    var elemento={
        producto:productos[parametro].item,
...
function cambiaCantidad(posicion, elemento){
    if(elemento.value<=0){
        Swal.fire({   title: "No puede ser cero!",   text: "Por favor la cantidad debe ser mayor a 0!",   
        icon: "warning"});
        return;
    }
...
function registrarCompra(){
    ...
    if (proveedor==''||fecha==''||comprobante ==''||total==0) {
        Swal.fire({   title: "Faltan datos!",   text: "Por favor complete todos los compos!",   
            icon: "warning"});
            return;
    }
    ...
    for (let i = 0; i < carrito.length; i++) {
        for (let j = 0; j < losProductos.length; j++) {
            if (losProductos[j].item==carrito[i].producto) {
...
function cargarDatos(){
    ...
    for(let i=0;i<compras.length;i++){
        cadena+=`<tr>
                    ...
                    <td>
                        <div class="acciones">
                            <button onclick="verCompra(${i})" class="btn btn-edit m5">
                                <i class="fa fa-eye"></i>
                            </button>
...
function eliminarCompra(posicion){
    ...
    Swal.fire({
        title: "Esta seguro?",
        text: "La compra se eliminará y se descontará del stock!",
        icon: "warning",
        ...
        for (let i = 0; i < laCompra.detalles.length; i++) {
            for (let j = 0; j < losProductos.length; j++) {
                if (losProductos[j].item==laCompra.detalles[i].producto) {
