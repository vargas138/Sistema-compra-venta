...
function registrarVenta(){
    ...
    if (cliente==''||fecha==''||comprobante ==''||total==0) {
        Swal.fire({   
            title: "Faltan datos!",   
            text: "Por favor complete todos los compos!",   
            icon: "warning"});
            return;
    }
...
function cargarDatos(){
    ...
    for(let i=0;i<ventas.length;i++){
        cadena+=`<tr>
                    ...
                    <td>
                        <div class="acciones">
                            <button onclick="verVenta(${i})" class="btn btn-edit m5">
                                <i class="fa fa-eye"></i>
                            </button>
...
function eliminarVenta(posicion){
    ...
    Swal.fire({
        title: "Esta seguro?",
        text: "La venta se eliminará y se regresará el producto al stock!",
        icon: "warning",
        ...
        for (let i = 0; i < laVenta.detalles.length; i++) {
            for (let j = 0; j < losProductos.length; j++) {
                if (losProductos[j].item==laVenta.detalles[i].producto) {
